...
public class ReviewLikeService {
    ...
    public void reviewLike (Long memberId, Long reviewId) {
        Member member = memberRepository.findById(memberId).orElseThrow( 
                () -> new ReviewLikeException(ErrorCode.MEMBER_NOT_FOUND)
        );
        Review review = reviewRepository.findById(reviewId).orElseThrow(
                () -> new ReviewLikeException(ErrorCode.REVIEW_LIKE_NOT_FOUND)
        );
    ...
    public void deleteReviewLike (Long memberId, Long reviewId) {
        ...
        Review review = reviewRepository.findById(reviewId).orElseThrow(
                () -> new ReviewLikeException(ErrorCode.REVIEW_LIKE_NOT_FOUND) // 나중에 리뷰로 변경
        );
    ...
    public List<ReviewLikeResponseDto> checkAllReviewLikes (Long memberId) { // 이 메서드는 외부에서 호출 가능한 public 메서드이며, Long 타입의 memberId(회원 ID)를 매개변수로 받아 해당 회원이 좋아요한 리뷰들을 ReviewLikeResponseDto 형태로 리스트에 담아 반환한다.
        Member member = memberRepository.findById(memberId).orElseThrow( // memberRepository에서 주어진 memberId로 회원을 조회하고, 조회 결과가 없으면 "해당 회원을 찾을 수 없습니다."라는 예외(ReviewLikeException)를 발생시킨다. 조회된 회원 객체는 = 연산자를 통해 오른쪽 결과값을 왼쪽 변수 member에 저장(할당)한다.
                () -> new ReviewLikeException(ErrorCode.MEMBER_NOT_FOUND)
        );
